    ...
    def _encode_and_cache(
        ...
        with torch.no_grad():
            for start in trange(0, len(items), batch_size):
                ...
                token_ids_list = [
                    item.get("context_token_ids", item["token_ids"])
                    if isinstance(
                        item.get("context_token_ids", item["token_ids"]), list
                    )
                    else item.get("context_token_ids", item["token_ids"]).tolist()
                ...
                for j, item in enumerate(batch_items):
                    ...
                    item["context_token_embeddings"] = arr
