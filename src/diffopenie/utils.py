...
def seed_everything(seed: int | None, *, deterministic: bool = False) -> None:
    """Seed Python, NumPy, and Torch for reproducible sampling.

    When `deterministic` is True, also enable deterministic Torch algorithms
    where possible. This is intended for inference/reproducibility paths.
    """
    if seed is None:
        return
    random.seed(seed)
