...
from diffopenie.models.discrete.denoiser import DiscreteDenoiser
...
class Trainer:
    def __init__(
        ...
        num_states = self.model.scheduler.num_states
        while len(weights) < num_states:
            if self.model.scheduler.kernel == "mask_absorbing":
                weights.append(float("inf"))  # heavily penalise predicting MASK
            else:
                weights.append(0.0)  # PAD: always ignored via ignore_index anyway
    ...
    def _to_device(self, batch: dict[str, torch.Tensor]):
        context_token_ids = batch.get("context_token_ids", batch.get("token_ids"))
        context_attention_mask = batch.get(
            "context_attention_mask", batch.get("attention_mask")
        )
        tag_ids = batch.get("tag_ids", batch.get("label_indices"))
        tag_attention_mask = batch.get("tag_attention_mask", batch.get("state_mask"))

        if context_token_ids is None:
            raise ValueError("Batch is missing context_token_ids/token_ids")
        if context_attention_mask is None:
            context_attention_mask = torch.ones_like(context_token_ids)
        if tag_ids is None:
            raise ValueError("Batch is missing tag_ids/label_indices")
        if tag_attention_mask is None:
            if context_attention_mask.shape == tag_ids.shape:
                tag_attention_mask = context_attention_mask
            else:
                tag_attention_mask = torch.ones_like(tag_ids)

        return (
            context_token_ids.to(self.device),
            context_attention_mask.to(self.device),
            tag_ids.to(self.device),
            tag_attention_mask.to(self.device),
        )
    ...
    def compute_loss(self, batch: dict[str, torch.Tensor]) -> ForwardResult:
        ...
        B = context_token_ids.shape[0]
        tag_len = tag_ids.shape[1]
        ...
        with torch.autocast(
            ...
            if isinstance(self.model.denoiser, DiscreteDenoiser) and (
                context_token_ids.shape[1] != tag_len
            ):
                raise ValueError(
                    "Grouped batches require CrossAttentionDenoiser: the "
                    "plain self-attention denoiser needs context and tag "
                    "sequences to have the same length."
                )

            token_emb = self.model.encode_tokens(
                context_token_ids, context_attention_mask
            )
            t = self.model.scheduler.sample_t(B)
            x_t = self.model.noise(tag_ids, t)
            state_mask = tag_attention_mask.to(context_attention_mask.dtype)
            logits = self.model.denoise(
                x_t, t, token_emb, context_attention_mask, state_mask
            )

            target = tag_ids.clone()
            target[tag_attention_mask == 0] = IGNORE_INDEX
            if self.model.scheduler.kernel == "mask_absorbing":
                target[x_t != self.model.scheduler.mask_state_id] = IGNORE_INDEX

            if self.background_drop_prob > 0 and self.model.training:
                bg_and_active = (tag_ids == SEQ_STR2INT["B"]) & (
                    target != IGNORE_INDEX
                )
                drop = bg_and_active & (
        ...
        metric_mask = tag_attention_mask.clone()
    ...
    def validate(
        ...
        for i, batch in enumerate(
            ...
            if isinstance(self.model.denoiser, DiscreteDenoiser) and (
                context_token_ids.shape[1] != tag_ids.shape[1]
            ):
                raise ValueError(
                    "Grouped batches require CrossAttentionDenoiser: the "
                    "plain self-attention denoiser needs context and tag "
            ...
            out = self.model.generate(
                batch_size=context_token_ids.shape[0],
                context=token_emb,
                context_attention_mask=context_attention_mask,
                tag_attention_mask=state_mask,
                return_intermediate=compute_per_timestep_metrics,
            )
            metric_mask = state_mask.clone()
            if compute_per_timestep_metrics:
                preds, intermediates = out
                metrics.update(preds, tag_ids, metric_mask)
                per_t_metrics.update(intermediates, tag_ids, metric_mask)
            else:
                metrics.update(out, tag_ids, metric_mask)
...
class TrainerConfig(BaseModel):
    ...
    max_grad_norm: float = 1.0
    background_drop_prob: float = 0.8
    class_weights: list[float] | None = None  # per-state weights for CE loss; None = equal
    label_smoothing: float = 0.0
    warmup_steps: int = 0
