...
def state_id_to_str(sid: int) -> str:
    return _STATE_ID_TO_STR.get(str(sid), "?")
...
_TAG_COLORS: dict[str, tuple[str, str]] = {
    ...
    "R": ("#d4a8d4", "#300"),
    "O": ("#90d490", "#030"),
    "M": ("#666", "#ccc"),  # mask_absorbing: absorbing MASK state
    "P": ("#666", "#ccc"),  # uniform: PAD state
}
_LABEL_NAMES = ["B", "S", "R", "O"]
_LABEL_NAMES_5 = ["B", "S", "R", "O", "M"]
_LABEL_NAMES_5_PAD = ["B", "S", "R", "O", "P"]
...
def build_diffusion_table(
    ...
    def _tag(s: int) -> str:
        t = state_id_to_str(s)
        return "P" if t == "M" and uniform else t
...
def _legend_html(uniform: bool = False) -> str:
    _label_text = {
        "B": "Background", "S": "Subject", "R": "Relation", "O": "Object",
        "M": "MASK", "P": "PAD",
    }
    tags = _LABEL_NAMES_5_PAD if uniform else _LABEL_NAMES_5
    parts = []
    for tag in tags:
...
def run_n_generations(
    ...
    with torch.no_grad():
        ...
        samples = model.generate(
            batch_size=n,
            context=token_emb,
            context_attention_mask=attn_rep,
            tag_attention_mask=attn_rep,
        )
...
def compute_entropy_series(
    ...
    for k in range(N):
        H = per_token_entropy(
            samples[: k + 1].clamp(0, num_classes - 1),
...
def build_distribution_frames(
    ...
    Returns: [N, L, C] array — probs[k, l, c] = P(label=c | token=l) after k+1 samples
    """
    N, L = samples.shape
    cumulative = torch.zeros(L, num_classes)
    all_probs = np.zeros((N, L, num_classes))
    for k in range(N):
        row = samples[k].clamp(0, num_classes - 1)  # [L]
...
def plot_distribution_animation(
    all_probs: np.ndarray,  # [N, L, C]
    token_labels: list[str],
    uniform: bool = False,
) -> go.Figure:
    N, L, _ = all_probs.shape
    x = list(range(L))
    if all_probs.shape[2] == 5:
        labels = _LABEL_NAMES_5_PAD if uniform else _LABEL_NAMES_5
    else:
        labels = _LABEL_NAMES
    colors = [_TAG_COLORS[label][0] for label in labels]

    def make_traces(k: int) -> list[go.Bar]:
        bottoms = np.zeros(L)
        traces = []
        for c, (label, color) in enumerate(zip(labels, colors)):
            traces.append(
                go.Bar(
                    x=x,
                    y=all_probs[k, :, c],
...
def page_debug(model, config, tokenizer, dataset, split, index,
    ...
    if need_gen:
        with st.spinner("Running diffusion..."):
            gen_seed = None if reroll else actual_seed
            x_0, intermediates, final_tags = run_generation(model, token_ids, gen_seed)
    ...
    final_tags = [_fix_tag(t) for t in final_tags]

    gt_tags: list[str] | None = (
        [_fix_tag(state_id_to_str(int(s))) for s in gt_labels] if gt_labels is not None else None
    )
    ...
    st.markdown(
        build_diffusion_table(tokens, gt_tags, intermediates, final_tags, T, uniform=uniform),
        unsafe_allow_html=True,
    )
    st.markdown(_legend_html(uniform=uniform), unsafe_allow_html=True)
...
def page_entropy(model, config, tokenizer, dataset, split, index,
    ...
    with st.spinner("Computing entropy..."):
        num_classes = model.scheduler.num_states
        all_probs = build_distribution_frames(samples, num_classes)
        entropy_matrix, mean_entropy, max_entropy = compute_entropy_series(
            samples, num_classes
        )
    ...
    with col_dist:
        st.plotly_chart(
            plot_distribution_animation(all_probs, tokens, uniform=uniform),
    ...
    if st.button("🎞  Build GIF (slow)"):
        with st.spinner("Rendering GIF..."):
            gif_bytes = make_distribution_gif(all_probs, tokens, uniform=uniform)
        st.download_button(
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="configs/lsoie_ex_2500/lsoie_ex_2500_config.yaml")
    parser.add_argument("--checkpoint", default="configs/lsoie_ex_2500/weights.pt")
...
def main() -> None:
    ...
    with st.sidebar:
        st.header("Model")
        config_path = st.text_input("Config path", cli.config)
        checkpoint_path = st.text_input("Checkpoint path", cli.checkpoint)
