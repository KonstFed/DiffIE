...
class DiscreteDenoiser(nn.Module):
    ...
    def forward(
        ...
        context: torch.Tensor,           # (B, L, ctx_dim) — must match L of x_t
        context_attention_mask: torch.Tensor,  # (B, L) accepted for unified API
        tag_attention_mask: torch.Tensor,      # (B, L) 1=keep, 0=hide from self-attn
    ) -> torch.Tensor:
        del context_attention_mask  # this denoiser fuses context per-position; full L is required
        B, L = x_t.shape
        if context.shape[:2] != (B, L):
            raise ValueError("context must have shape (B, L, ctx_dim)")
        if tag_attention_mask.shape != (B, L):
            raise ValueError("tag_attention_mask must have shape (B, L)")

        # PyTorch MHA uses key_padding_mask with True at pads
        key_padding_mask = tag_attention_mask == 0  # (B, L) bool

        x_state = self.state_embed(x_t)  # (B, L, D)
        x_ctx = self.ctx_proj(context)  # (B, L, D)
        t_emb = (
...
class DiscreteDenoiserConfig(BaseModel):
    ...
    type: Literal["self_attn"] = "self_attn"
    num_states: int  # K
    model_dim: int  # D
    ctx_dim: int  # D_ctx (dimension of token_embeddings)
    num_layers: int = 4
