...
class BERTEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.add_special_tokens:
            ...
            cls_tokens = torch.full((B, 1), self.cls_token_id, dtype=torch.long, device=device)
            sep_tokens = torch.full((B, 1), self.sep_token_id, dtype=torch.long, device=device)
            input_ids_with_special = torch.cat([cls_tokens, input_ids, sep_tokens], dim=1)  # [B, L+2]
        ...
        token_embeddings = outputs.last_hidden_state  # [B, L+2, bert_dim] or [B, L, bert_dim]
